...
<template>
  <div>
    <h2 class="mt-10">Modules outside the Curriculum</h2>
    <div v-if="description" class="mb-5" v-html="description.content" />
    <div class="flex flex-col gap-5">
      <div v-for="(module, index) in modulesOutsideArray" :key="index" class="flex gap-10">
        <Input v-model="module.title" label="Module Title" />
        <NumberInput v-model="module.ects" label="ECTS" :min="0" type="number" />
        <Input v-model="module.university" label="University" />
...
interface Props { texts: IText[] }
interface Emits { updateModulesOutsideData: [value: IModuleOutside[]] }
const props = defineProps<Props>(),
 emit = defineEmits<Emits>(),
 description: IText | undefined =
  props.texts.find((text) => text.name === 'modules_outside_description') || undefined,
 modulesOutsideArray: Ref<IModuleOutside[]> = ref([]);
